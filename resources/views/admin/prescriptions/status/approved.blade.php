            ...
            <div class="col-md-12 col-lg-7">
                <div class="card card-default">
                    <div class="card-header card-header-border-bottom justify-content-between">
                        <h2>Chi tiết đơn hàng <small>#{{ $prescription->order->id }}</small></h2>
                    </div>
                    <div class="card-body">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($prescription->order->orderDetails as $key => $orderDetail)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $orderDetail->medicine->name }}</td>
                                    <td>{{ $orderDetail->medicine->unit }}</td>
                                    <td>{{ $orderDetail->quantity }}</td>
                                    <td class="text-right">{{ number_format($orderDetail->medicine->price) }} <u>đ</u>
                                    </td>
                                    <td class="text-right">{{ number_format($orderDetail->total) }} <u>đ</u></td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                        <div id="total-confirm" class="row justify-content-end">
                            <div class="col-lg-5 col-xl-4 col-xl-3 ml-sm-auto">
                                <ul class="list-unstyled mt-4">
                                    <li class="pb-3 text-dark">{{ __('labels.total') }}
                                        <span
                                            class="d-inline-block float-right total">{{ number_format($prescription->order->total) }}
                                            <u>đ</u></span>
                                    </li>
                                </ul>
                            </div>
